...
# @FileName  : merger.py
# @Project   : ODPlatform
# @Function  : Merge runtime configuration sources with field-level provenance.
...
class ConfigMetadata:
    ...
    def chain(self) -> List["ConfigMetadata"]:
        result: list[ConfigMetadata] = [self]
    ...
    def chain_str(self) -> str:
        return " ← ".join(f"{meta.value}({meta.source_label})" for meta in self.chain())
...
class ConfigMerger:
    """Merge DEFAULT/YAML/CLI config sources and keep provenance metadata."""
    ...
    def merge(self, config_class, *, sources=None):
        ...
        try:
            ...
            if self.track_sources:
                self._enhance_validation_error(exc)
            raise

    def preview(self, config_class, *, sources=None) -> Dict[str, Any]:
        """Dry-run merge: keep provenance but do not instantiate or validate."""
    ...
    def _do_merge(self, config_class, sources) -> Dict[str, Any]:
        ...
        merged: Dict[str, Any] = {}
        for source, config in all_sources:
            self._apply_source(merged, dict(config or {}), source)
        return merged

    def _apply_source(self, merged: Dict[str, Any], config: Dict[str, Any], source) -> None:
    ...
    def get_source_report(self) -> str:
        ...
        builtin = [source.value for source in (ConfigSource.CLI, ConfigSource.YAML, ConfigSource.DEFAULT)]
        ordered = [source for source in builtin if source in by_source]
        custom = sorted(source for source in by_source if source not in builtin)
    ...
    def get_override_report(self) -> str:
        ...
        for key in dict.fromkeys(self._overridden_keys):
    ...
    def _is_sensitive(self, key: str) -> bool:
        ...
        return bool(
            cls
            and hasattr(cls, "sensitive_field_names")
    ...
    def to_audit_log(self) -> Dict[str, Any]:
        return {
            "fields": {
                key: {
                    "value": self._display_value(key, meta.value),
                    "source": meta.source_label,
                    "chain": [f"{item.value}({item.source_label})" for item in meta.chain()],
                }
                for key, meta in self._metadata.items()
            },
    ...
    def _enhance_validation_error(self, exc: ValidationError) -> None:
        try:
            for error in exc.errors():
                ...
                meta = self._metadata.get(key)
                if meta:
                    exc.add_note(f"[来源] {key}: {meta.chain_str()}")
        except Exception:
            pass
