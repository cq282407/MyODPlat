...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Runtime train yaml; defaults to configs/runtime/train.yaml",
    )
    ...
    parser.add_argument(
        "--batch",
        type=_batch_value,
        help="Batch size, -1, or GPU memory fraction",
    )
    ...
    parser.add_argument(
        "--run-id",
        type=str,
        help="ODPlatform stable run id for logs, reports and archives",
    )
    parser.add_argument("--operator", type=str, help="Operator name written to the training report")
    parser.add_argument(
        "--operator-role",
        type=str,
        help="Operator role/team written to the training report",
    )
    parser.add_argument("--notes", type=str, help="Free-form notes written to the training report")
    parser.add_argument(
        "--no-pre-validate",
        dest="pre_validate",
        action="store_false",
    ...
    parser.add_argument(
        "--academic-plots",
        action="store_true",
        help="Apply matplotlib academic style for this process",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
...
def main(argv: list[str] | None = None) -> int:
    ...
    get_logger(
        ...
        run_id=args.run_id or args.name,
    )
    ...
    cli_args = {
        ...
        if value is not None and key not in non_config
    }
    ...
    if result.success:
        log.info(
            "训练成功: output=%s time=%.2fs",
            result.output_dir,
            result.train_time or 0.0,
        )
        return 0
